...
params.num_alphas = 10
params.num_etas = 10
params.range_alphas = [0,1]
...
process PREP_DATA {
    ...
    random.seed(42)
    sampled_names = random.sample(hao_rna.obs_names, round(data_subset * hao_rna.n_obs))
...
process SGD_REGRESS {
    ...
    for a in [adts_in_cbmc[0]]:
        SGDmodels[a] = SGDRegressor(loss='squared_error', penalty = 'l1', alpha = 0.015, learning_rate='constant', eta0=1e-5) #0.015 is the mean of the alpha values from training ADTs in PBMC dataset from Stoeckius
...
alphas_ch.combine(etas_ch).view()
}
